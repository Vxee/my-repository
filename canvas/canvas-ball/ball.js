var canvas = document.getElementById('cas'),
    ...
    vpy = canvas.height/2,
    Radius = 50,  // 整体大球的半径
    LayerBallNum = 360 / 20, //横向圆周个数
    LayerIntervalUp = 360 / 20, // 纵向
    balls = [],
    angleX = Math.PI / 100,
    angleY = Math.PI / 100;
...
Animation.prototype = {
    isrunning: false,
    init: function() {
        var num = LayerIntervalUp / 2; //layer 的数目
        for(var i = 0; i <= num; i++){
            var l = new layer(i,1);
            l.draw();
            var l = new layer(i, -1);
...
layer.prototype = {
    setBalls: function(radius){
        for(var i = 0; i < LayerBallNum; i++){
            ...
            var b = new ball(radius * Math.cos(angle), radius * Math.sin(angle), this.up * Math.sqrt(Math.pow(Radius, 2) - Math.pow(radius, 2)),0.8);
            b.paint();
            balls.push(b);
        }
    },
...
ball.prototype = {
    paint: function(){
        ...
        ctx.arc(vpx + this.x, vpy + this.y, this.r, 0, 2 * Math.PI, true);
...
document.getElementById('controllBtn').onclick = function(){
